...
let rec map_tlrec f list =
  ...
  in
  map_tlrec' f list [] (*mamo tudi list.rev namest reverse *)
...
let rec mapi f list =
  let mapi' f list mapi_list index =     (*kjer mapi_list = acc in i = index*)
    match list with
    | [] -> mapi_list  (*reverse?*)
    | x :: xs -> 
...
let rec mapi f list =
  let mapi' f list mapi_list index =     
    match list with
    | [] -> mapi_list
...
let rec unzip_tlrec list =
  let unzip_tlrec' list xacc yacc=
    match list with
    | [] -> (reverse xacc, reverse yacc)
    | (x, y) :: xys -> unzip_tlrec' xys (x :: xacc) (y :: yacc)
  in
  unzip_tlrec' list [] []
...
let rec fold_left_no_acc f list =
  match list with
  | x :: [] -> x
  | x :: y :: xs -> let a = f x y in fold_left_no_acc f (a::xs)
...
let rec apply_sequence f x n =
  let apply_sequence' f x n' acc =
    x :: apply_sequence f x (n'-1)
