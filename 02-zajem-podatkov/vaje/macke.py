...
def download_url_to_string(url):
    """Funkcija kot argument sprejme niz in poskuša vrniti vsebino te spletne
...
def page_to_ads(page_content):
    ...
    return re.findall(exp, page_content, re.DOTALL) #da tudi nove vrstice ujame
...
def get_dict_from_ad_block(block):
    ...
    """
    exp = r'<h3><a title=".*?" href=".*?">(?P<naslov>.*?)</a></h3>\s*(?P<opis>.*?)\s*<div.*<b>(?P<rodovnik>.*?)</b>.*<div class="price">(?P<cena>.*?)</div>'
